...
//
//  JM: Doing scare_mouse() inside MSG_DRAW events can lock the program.
//      It can't help that each time MSG_DRAW is called manually, the mouse
//	is already being hidden.
//

#define SCARE_MOUSE()
#define UNSCARE_MOUSE()

//
//  JM: This actually deadlocks the program if we use acquire_screen()...
//

#define ACQUIRE_SCREEN()
#define RELEASE_SCREEN()
//#define ACQUIRE_SCREEN() acquire_screen();
//#define RELEASE_SCREEN() release_screen();
...
//    scare_mouse();
    ...
    for (; p<=b; p++) {
      ...
      if (d->flags & D_DISABLED)
        ...
        bg=(f ? fg : get_bill_color(bill_framebg));
      textout_ex(screen, font, buf, d->x+x+3, d->y+3, f ? get_bill_color(bill_framebg) : fg,bg);
      x += w;
    }
...
int d_billtext_proc(int msg, DIALOG *d, int c)
{
 ...
 if (msg==MSG_DRAW) {
  ...
  gui_textout_ex(screen, d->dp, d->x, d->y, fg, bg, FALSE);
  UNSCARE_MOUSE();
 }
 return D_O_K;
}
...
//  scare_mouse();
  ...
  gui_textout_ex(screen, d->dp, d->x, d->y, fg,bg, TRUE);
  //unscare_mouse();
  UNSCARE_MOUSE();
 }
 return D_O_K;
...
//  scare_mouse();
  ...
  {
   x = d->x +
       gui_textout_ex(screen, d->dp, d->x, d->y+(d->h-(text_height(font)-gui_font_baseline))/2, fg,bg, FALSE) +
       text_height(font)/2;
  } 
  else   /* Check is to the left */
  {
   x = d->x;
   gui_textout_ex(screen, d->dp, d->x + d->h + text_height(font)/2,d->y+(d->h-(text_height(font)-gui_font_baseline))/2, fg, bg, FALSE);
  }
